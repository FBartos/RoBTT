...
model {
  ...
  if(is_ss == 0){
    for(i in 1:N1){
      if(x1[i] == 0){
        target += bernoulli_lpmf(0 | mu_i[1]);
      }else if(x1[i] == 1){
        target += bernoulli_lpmf(1 | mu_i[1]);
      }else{
        target += beta_lpdf(x1[i]  | alpha_i[1], beta_i[1]);
      }
    }
    for(i in 1:N2){
      if(x2[i] == 0){
        target += bernoulli_lpmf(0 | mu_i[2]);
      }else if(x2[i] == 1){
        target += bernoulli_lpmf(1 | mu_i[2]);
      }else{
        target += beta_lpdf(x2[i]  | alpha_i[2], beta_i[2]);
      }
    }
  }else{
