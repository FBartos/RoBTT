...
parameters{
  ...
  real<lower = coefs_lb(bounds_type_p[1], bounds_p[1]), upper = coefs_ub(bounds_type_p[2], bounds_p[2])> p[is_p];
}
transformed parameters {
  ...
  if(is_p == 1){
    prop_i[1] = exp(beta + 0.5 * p[1]) / (exp(beta + 0.5 * p[1]) + 1);
    prop_i[2] = exp(beta - 0.5 * p[1]) / (exp(beta - 0.5 * p[1]) + 1);
  }else{
    prop_i[1] = exp(beta + 0.5 * fixed_p[1]) / (exp(beta + 0.5 * fixed_p[1]) + 1);
    prop_i[2] = exp(beta - 0.5 * fixed_p[1]) / (exp(beta - 0.5 * fixed_p[1]) + 1);
  }
...
model {
  ...
  if(is_p == 1){
    target += set_prior(p[1], prior_type_p, prior_parameters_p, bounds_type_p, bounds_p);
  }
  ...
  if(is_ss == 0){
    for(i in 1:N1){
      if(x1[i] != 0){
        target += bernoulli_lpmf(1 | prop_i[1]);
        target += lognormal_lpdf(x1[i] | mu_log_i[1], sigma_log_i[1]) + log(prop_i[1]);
      }else{
        target += bernoulli_lpmf(0 | prop_i[1]);
      }
    }
    for(i in 1:N2){
      if(x2[i] != 0){
        target += bernoulli_lpmf(1 | prop_i[2]);
        target += lognormal_lpdf(x2[i] | mu_log_i[2], sigma_log_i[2]) + log(prop_i[2]);
      }else{
        target += bernoulli_lpmf(0 | prop_i[2]);
      }
    }
  }else{
