#' @title Creates a RoBTT prior
#'
#' @description \code{prior} creates a prior distribution for fitting
#' a RoBTT model. The prior can be visualized by a \code{plot} function.
#'
...
#' [RoBTT()] creates models corresponding to all combinations of prior
...
#' # (see ?plot.prior.RoBTT for all options)
#' plot(p1)
#'
#'
#' @export  prior
#' @rawNamespace S3method(print, RoBTT.prior)
#' @seealso [plot.RoBTT.prior()], \link[stats]{Normal}, \link[stats]{Lognormal}, \link[stats]{Cauchy},
...
prior <- function(distribution, parameters, truncation = list(lower = -Inf, upper = Inf), prior_odds = 1){
  ...
  if(distribution %in% c("norm", "normal")){
    ...
    if(truncation$lower == -Inf)truncation$lower <- 0 # change the default lower truncation
    ...
    if(!is.null(names(parameters))){
      if(!all(names(parameters) %in% c("a", "b")))stop(paste0("Parameters ", paste(names(parameters)[!names(parameters) %in% c("a", "b")], sep = ", ", collapse = ""), " are not supported for a uniform distribution."))
    }else{
    ...
    if(length(parameters) != 2)stop("Beta prior distribution requires 2 parameters.")
    if(!is.null(names(parameters))){
      if(!all(names(parameters) %in% c("alpha", "beta")))stop(paste0("Parameters ", paste(names(parameters)[!names(parameters) %in% c("alpha", "beta")], sep = ", ", collapse = ""), " are not supported for a beta distribution."))
    }else{
      names(parameters) <- c("alpha", "beta")
    }
    if(parameters$alpha <= 0)stop("Parameter 'alpha' must be positive.")
    if(parameters$beta  <= 0)stop("Parameter 'beta' must be positive.")
    
    if(truncation$lower == -Inf)truncation$lower <- 0 # change the default lower truncation
    if(truncation$upper ==  Inf)truncation$upper <- 1 # change the default upper truncation
    if(truncation$lower < 0)stop("Lower truncation point must be larger or equal to 0.")
    if(truncation$upper > 1)stop("Upper truncation point must be smaller or equal to 1.")
    ...
    if(length(parameters) != 1)stop("Exponential prior distribution requires 1 parameters.")
    if(!is.null(names(parameters))){
      if(!all(names(parameters) %in% c("shape", "rate")))stop(paste0("Parameters ", paste(names(parameters)[!names(parameters) %in% c("shape", "rate")], sep = ", ", collapse = ""), " are not supported for an exponential distribution."))
    }else{
      names(parameters) <- c("rate")
    }
    if(!is.null(parameters$scale)){
      parameters$rate  <- 1/parameters$scale
      parameters$scale <- NULL
  ...
  class(output) <- "RoBTT.prior"

  return(output)
}


#' @title Prints a RoBTT.prior object
#'
#' @param x a RoBTT prior
...
#'  }
#' @export  print.RoBTT.prior
#' @rawNamespace S3method(print, RoBTT.prior)
#' @seealso [prior()]
print.RoBTT.prior <- function(x, ...){
  ...
  }else if(x$distribution == "beta"){
    parameters <- c(x$parameters$alpha, x$parameters$beta)
  }else if(x$distribution == "exp"){
    parameters <- c(x$parameters$rate)
  }
...
#' @title Plots a RoBTT.prior object
#'
#' @param x a RoBTT prior
...
#' for odds ratios).
#' @param transformation whether and how should the prior distribution
...
#' @param ... additional arguments
#' @export  plot.RoBTT.prior
#' @rawNamespace S3method(plot, RoBTT.prior)
#' @seealso [prior()]
plot.RoBTT.prior <- function(x, plot_type = "base", effect_size = NULL, transformation = NULL,
                             show_figures = -1, weights = FALSE, par_name = "mu",
                             samples = 1e6, points = 1000, ...){

  # check input
  if(plot_type == "ggplot2")plot_type <- "ggplot"
  if(!par_name %in% c("mu", "tau", "omega", "theta"))stop("The passed 'par_name' is not supported for plotting. See '?plot.RoBTT' for more details.")
  if(!plot_type %in% c("base", "ggplot"))stop("The passed 'plot_type' is not supported for plotting. See '?plot.RoBTT' for more details.")
  ...
  if(!is.null(effect_size) & !is.null(par_name)){
    if(par_name == "mu"){
      if(effect_size == "r"){
        if(is.null(transformation)) transformation <- "cohens_d"
      }else if(effect_size == "OR"){
        if(is.null(transformation)) transformation <- "log_OR"
      }
    }
  }
  if(is.null(par_name)){
  ...
  plot_data <- .plot.prior_data(x, samples, points, weights, par_name, effect_size, transformation)
...
.plot.prior_data <- function(prior, samples = 1e6, points = 1000, weights = FALSE, par_name = "", effect_size = NULL, transformation = NULL, x_range = NULL, temp_x = NULL){
  ...
  if(!is.null(effect_size) & !par_name %in% c("tau", "omega")){
    if(effect_size == "r"){
      if(prior$distribution == "point"){
        ...
        transformed_trunc <- list(
          from = if(!is.infinite(prior$truncation$lower)) .transform(prior$truncation$lower, effect_size, transformation) else -1,
          to   = if(!is.infinite(prior$truncation$upper)) .transform(prior$truncation$upper, effect_size, transformation) else  1
        )
      }
    }else if(effect_size == "OR"){
      if(prior$distribution == "point"){
        ...
        transformed_trunc <- list(
          from = if(!is.infinite(prior$truncation$lower)) .transform(prior$truncation$lower, effect_size, transformation) else  0,
          to   = if(!is.infinite(prior$truncation$upper)) .transform(prior$truncation$upper, effect_size, transformation) else  Inf
        )
      }
    }else{
      transformed_trunc <- list(
  ...
  if(!is.null(temp_x)){
    ...
    if(par_name == "omega" & !weights){
      ...
      prob[[1]] <- TRUE
      x_range   <- c(max(c(prior$truncation$lower, .transform(prior$parameters$location, effect_size, transformation) - .5)),
                     min(c(prior$truncation$upper, .transform(prior$parameters$location, effect_size, transformation) + .5)))

      temp_x    <- rep(prior$parameters$location, 2)
      temp_x    <- .transform(temp_x, effect_size, transformation)
      df[[1]]   <- data.frame(
        x = temp_x,
        y = c(0, 1)
      )
    ...
    if(is.null(transformation)){
      ...
      df[[1]] <- data.frame(
        x = temp_x,
        y = exp(stats::dlnorm(temp_x, meanlog = prior$parameters$meanlog, sdlog = prior$parameters$sdlog, log = TRUE) -
                  log(
                    stats::plnorm(prior$truncation$upper, prior$parameters$meanlog, prior$parameters$sdlog, lower.tail = TRUE, log.p = FALSE) -
                      stats::plnorm(prior$truncation$lower, prior$parameters$meanlog, prior$parameters$sdlog, lower.tail = TRUE, log.p = FALSE)
                  ))
      )
    }else{
      # sampling for transformed parameters
      ...
      temp_x  <- NULL
      while(length(temp_x) < samples){
        new_temp_x <- stats::rlnorm(samples, meanlog = prior$parameters$meanlog, sdlog = prior$parameters$sdlog)
        new_temp_x <- new_temp_x[new_temp_x > prior$truncation$lower & new_temp_x < prior$truncation$upper]
        temp_x     <- c(temp_x, new_temp_x)
      }
  ...
  if(prior$distribution %in% c("normal", "lognormal", "t", "gamma", "invgamma", "uniform")){
  ...
  if(length(names) == 0){
    ...
    if(par_name == ""){
      ...
      if(is.null(effect_size)){
        ...
        names[[1]] <- bquote(
          ~rho~(
            .(if(transformation == "cohens_d") bquote("Cohen's"~italic("d")) else if(transformation == "fishers_z") "Fisher's"~italic("z"))
            ~"~"~.(print(prior, plot = TRUE))))
      }else if(effect_size == "OR"){
        names[[1]] <- bquote(
          ~italic("OR")~(
            .(if(transformation == "cohens_d") bquote("Cohen's"~italic("d")) else if(transformation == "log_OR") bquote("log"(italic("OR"))))
            ~"~"~.(print(prior, plot = TRUE))))
