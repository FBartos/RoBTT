...
fit1_trunc <- RoBTT(
  ...
  seed = 1, parallel = FALSE)
```
#TODO: there should be some summary already 
...
```{r}
summary(fit2_trunc, group_estimates = TRUE)
```
...
We can also summarize information about the specified models by setting the `type = "models"` argument. 

```{r}
summary(fit2_trunc, group_estimates = TRUE, type = "models")
```
