...
author: "Maximilian Maier & Frantisek Bartos"
date: "08/03/2022"
...
print_conditional <- function(fit, coef = "d"){
  with(fit, 
       sprintf("%1$s, 95%% CI [%2$s, %3$s]",
               format(round(mean(RoBTT$samples$conditional[[coef]]), 2), nsmall = 3),
               format(round(quantile(RoBTT$samples$conditional[[coef]], 0.025), 2), nsmall = 2),
               format(round(quantile(RoBTT$samples$conditional[[coef]], 0.975), 2), nsmall = 2)
       ))
} 
```
...
```{r}
library(RoBTT)
data <- read.csv2("FisherData.csv")
data$Crossed <- as.numeric(data$Crossed)
data$Self    <- as.numeric(data$Self)
```
...
fit_01 <- RoBTT(
  ...
  prior_r  = NULL, #this indicates no prior on the variance allocation factor -> equal variance test
  likelihood = "normal",
  seed     = 0
)
```
...
fit_10 <- RoBTT(
  ...
  parallel = TRUE,
  prior_d  = prior("cauchy", list(0, 1/sqrt(2))),
  prior_r  = prior("beta",   list(3, 3)), #prior on variance allocation
  prior_r_null = NULL, #remove models assuming equal variance
  likelihood = "normal",
  seed     = 0
)
```
...
fit_3 <- RoBTT(
  ...
  x2       = data$Crossed,
  parallel = TRUE,
  prior_d      = prior("cauchy", list(0, 1/sqrt(2)), list(0, Inf)),
  prior_r      = prior("beta",   list(3, 3)),
  prior_nu     = prior("exp",    list(1)),
  prior_d_null = prior("normal", list(0, 0.15), list(0, Inf)), #prior distribution and truncation
